...
[WARNING]
====
If you don't have performance issues in computing the Y coordinate, it is recommended to only store the X coordinate in your persistence system.
...
The other use cases will be shown in only one of the two ways.
...
As a REST API, I want to get the pseudonym at rest (always the same for a citizen in a pseudonymisation domain) from a pseudonym in transit to find information about a citizen in my database.
...
== Preloading of the known domains

If you don't want the first calls to `PseudonymisationHelper` to be slow because of the fetching of the domain, you can preload the domains your application knows by calling `getDomain("yourdomain_v1").get()`.

If `get()` throws an exception, you can decide to prevent the application from starting (because you suspect the problem is caused by a wrong configuration) or to continue with unloaded domains.

If you decide to continue the deployment, you should asynchronously and periodically (once per minute, for example) try to reload the domain by calling `refreshDomain("yourdomain_v1").get()` until `get()` does not throw any exception.

.Preloading example with retry
[source,java]
----
try {
    pseudonymisationHelper.getDomain("yourdomain_v1").get();
} catch (Exception e) {
    log.warn("Domain preloading failed, will retry periodically", e);
    // Schedule asynchronous periodic retry using refreshDomain() here
}
----

== Refreshing the domains you own every 6 hours

Because of the domain secret keys rotation, eHealth requires you to refresh the domains you own every 6 hours.

To achieve this, you can call `refreshDomain("yourdomain_v1").get()` asynchronously for every domain you own.

You can call `refreshableDomains()` to retrieve the set of domains you own that `PseudonymisationHelper` has previously loaded.

If the refresh fails, you should asynchronously and periodically (once per minute, for example) retry by calling `refreshDomain("yourdomain_v1").get()` until `get()` does not throw any exception.

WARNING: If a domain is not refreshed within the expected timeframe, the secret keys may become outdated, and operations on pseudonyms in transit for this domain may fail.

[IMPORTANT]
====
If the library throws an `UnknownKidException`, it can be caused by:

* *The domain has not been refreshed in time* — ensure you refresh your domains every 6 hours.
* *The received `PseudonymInTransit` was created with an expired secret key* — ask the sender to refresh their domain and to do not reuse expired `PseudonymInTransit`.
====
