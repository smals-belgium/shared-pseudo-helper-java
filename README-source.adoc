...
=== Resolve an SSIN from a pseudonym in transit from a pseudo-unaware domain

As a REST API, I want to get the SSIN from a pseudonym in transit from a pseudo-unaware domain (i.e., a domains where `pseudonymize` and `identify` operations are never used, as I am authorized to access the SSIN) to find information about a citizen in my database.
